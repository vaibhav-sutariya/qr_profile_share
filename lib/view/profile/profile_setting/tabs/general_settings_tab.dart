import 'package:flutter/material.dart';
import 'package:provider/provider.dart';
import 'package:qr_profile_share/configs/colors/app_colors.dart';
import 'package:qr_profile_share/configs/components/custom_elevated_button.dart';
import 'package:qr_profile_share/view/profile/profile_setting/tabs/widgets/build_arrowtile_widget.dart';
import 'package:qr_profile_share/view/profile/profile_setting/tabs/widgets/build_divider_widget.dart';
import 'package:qr_profile_share/view/profile/profile_setting/tabs/widgets/build_dropdowntile_widget.dart';
import 'package:qr_profile_share/view/profile/profile_setting/tabs/widgets/build_switchtile_widget.dart';
import 'package:qr_profile_share/view_model/controller/profile/profile_setting_view_model.dart';

class GeneralSettingsTab extends StatelessWidget {
  const GeneralSettingsTab({super.key});

  @override
  Widget build(BuildContext context) {
    return Consumer<ProfileSettingViewModel>(
      builder: (context, provider, child) {
        return Padding(
          padding: const EdgeInsets.symmetric(horizontal: 16.0),
          child: Column(
            children: [
              Align(
                alignment: Alignment.topCenter,
                child: Container(
                  decoration: BoxDecoration(
                    color: AppColors.whiteColor,
                    borderRadius: BorderRadius.circular(12),
                    boxShadow: [
                      BoxShadow(
                        color: AppColors.blackColor.withOpacity(0.08),
                        blurRadius: 6,
                        spreadRadius: 1,
                        offset: const Offset(0, 2),
                      ),
                    ],
                  ),
                  child: Padding(
                    padding: const EdgeInsets.symmetric(
                      horizontal: 16.0,
                      vertical: 12,
                    ),
                    child: Column(
                      mainAxisSize: MainAxisSize.min,
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text(
                          'General Settings',
                          style: TextStyle(
                            fontSize: 18,
                            fontWeight: FontWeight.w600,
                            color: AppColors.blackColor,
                          ),
                        ),
                        const SizedBox(height: 16),

                        /// Smart Auto-Update Contact
                        BuildSwitchtileWidget(
                          icon: Icons.edit,
                          title: 'Smart Auto-Update Contact',
                          value: provider.autoUpdateContact,
                          onChanged:
                              (val) => provider.toggleAutoUpdateContact(val),
                        ),

                        BuildDividerWidget(),

                        /// NFC Contact Sharing
                        BuildSwitchtileWidget(
                          icon: Icons.smartphone,
                          title: 'NFC Contact Sharing',
                          value: provider.nfcSharing,
                          onChanged: (val) => provider.toggleNfcSharing(val),
                        ),

                        BuildDividerWidget(),

                        /// Notification Preferences
                        BuildArrowtileWidget(
                          icon: Icons.notifications_none,
                          title: 'Notification Preferences',
                          onTap:
                              () =>
                                  provider.goToNotificationPreferences(context),
                        ),

                        BuildDividerWidget(),

                        /// Industry Template
                        BuildDropdowntileWidget(
                          icon: Icons.card_giftcard,
                          title: 'Industry Template',
                          value: provider.industry,
                          onChanged: provider.setIndustry,
                          items: const [
                            'Tech',
                            'Finance',
                            'Health',
                            'Education',
                          ],
                        ),
                        BuildDividerWidget(),
                      ],
                    ),
                  ),
                ),
              ),
              const SizedBox(height: 20),
              CustomElevatedButton(text: 'Save Changes', onPress: () {}),
            ],
          ),
        );
      },
    );
  }
}
