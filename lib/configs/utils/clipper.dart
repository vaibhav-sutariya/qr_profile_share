...
class ConcentricClipper extends CustomClipper<Path> {
  ...
  Rect _createGrowingShape(Path path, Size size) {
    double scaledProgress = progress * growFactor;
    double scaledLimit = limit * growFactor;
    double r = radius + pow(2, scaledProgress);
    double delta = (1 - scaledProgress / scaledLimit) * radius;
  ...
  Rect _createShrinkingShape(Path path, Size size) {
    double scaledProgress = (progress - limit) * growFactor;
    double scaledLimit = limit * growFactor;
    double r = radius + pow(2, scaledLimit - scaledProgress);
    double delta = scaledProgress / scaledLimit * radius;
    double x = size.width / 2 - r + delta;
    double y = (size.height * verticalPosition) + radius;
